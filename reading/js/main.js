...
for (i = 0; i < coll.length; i++) {
  coll[i].addEventListener("click", function() {
    ...
    if (this.innerHTML.indexOf("▼") !== -1) {
      this.innerHTML = "&#9654 Some comments about the data for those interested."
    } else {
      this.innerHTML = "&#x25BC Some comments about the data for those interested."
    }
...
d3.csv("data/gr_js_count.csv", function(error, data) {
  ...
  const division2 = new Date("2021-08-14")
  const division3 = new Date("2022-04-01")
...
function summer2016() {
  ...
  d3.csv("data/gr_js_count.csv", function(error, data) {
    ...
    .append("rect")
      ...
      d3.select(".shading.summer-16").transition().duration(1000).attr("opacity", 0.2)
  ...
  d3.csv("data/gr_js_count.csv", function(error, data) {
      ...
      summers.forEach(function (d) {
       ...
       chart
        ...
        d3.selectAll(".shading.college").transition().delay(500).duration(1000).attr("opacity", 0.2)
...
function drawPagesChart() {
  var chart = d3.select(".inner-chart")
  .append('svg')
...
function main() {
  ...
  new Waypoint({
    ...
    handler: function() {
        d3.selectAll('.divisions').transition().duration(1000).style("opacity", '1')
    },
    offset: offset
  });
  ...
  new Waypoint({
    ...
    handler: function(direction) {
      if (direction == 'down') {
        // 7 start grad school
        d3.selectAll(".shading").transition().attr("opacity", 0);
